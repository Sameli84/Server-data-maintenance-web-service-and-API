...
//    }
    ...
    public Course createCourse(CourseSchemaDto courseSchemaDto, Account account) {
        ...
        for (int i = 0; i < courseSchemaDto.getParts().size(); i++) {
            ...
            CourseSchemaPart part = modelMapper.map(p, CourseSchemaPart.class);
            part.setCourse(course);
            part.setOrder(i);

            // var part = new CourseSchemaPart(p.getName(), p.isValidator(), p.getValidatorRegex(), p.getValidatorMessage(), p.isLocked(), p.isAutogenerated(), p.getGenerationStatement(), p.getControlWidth(), i, p.isRequired(), course);
